...
class MasterBlaster_IntegrationService extends BaseApplicationComponent
{
	...
	private function _cleanUpEvents()
	{
		...
		foreach($this->existing_events as $event)
		{
					...
					|| ! $plugin_events[$event->registrar][$event->event])
			{
				...
				craft()->db->createCommand()->delete('masterblaster_notification_events', array('registrar' => $event->registrar));
				
				// disassociate
				craft()->db->createCommand()->delete('masterblaster_campaign_notification_events', array('notificationEventId' => $event->id));
			}
		}
	}
	...
	private function _registerEvent($event)
	{
		...
		{
			$new_event = MasterBlaster_NotificationEventRecord::model()->findByPk($existing_event);

		}	
		else // if doesn't exist, create
		{
			$new_event = new MasterBlaster_NotificationEventRecord();	
	...
	private function _getPluginEvents($plugin)
	{
		...
		if( ! is_dir($dir . 'integrations/masterblaster'))
		{
			return false;
		}
		...
		foreach (scandir($dir . 'integrations/masterblaster') as $file)
		{
			if ($file !== '.' and $file !== '..')
			{
				$parts = explode('.', $file);
				...
				{
					$base_class_files[$class] = $dir . 'integrations/masterblaster/' . $file;
				}
				else 
				{
					$plugin_files[$class] = $dir . 'integrations/masterblaster/' . $file;
				}
			}
		}
